...
//娱乐论坛启用新验证码
/*
if(strpos($_SERVER['HTTP_HOST'],"ent.sina") != FALSE){
 ...
 *   ["height"] => int(60)
 *   ["type"] => string(1) "0"
 *   ["background"] => string(1) "1"
 *   ["adulterate"] => string(1) "1"
 *   ["ttf"] => string(1) "0"
 *   ["angle"] => string(1) "0"
 *   ["color"] => string(1) "1"
 *   ["size"] => string(1) "0"
...
Common::debug("seccode : " , $seccode);
if($update) {
	$seccode = random(6, 1) + $seccode{0} * 1000000;
	updatesession();
}

Common::debug("seccode1 : " , $seccode);
seccodeconvert($seccode);

Common::debug("seccode2 : " , $seccode);
if(!$nocacheheaders) {
	@dheader("Expires: -1");
...
if ($seccodedata['type'] < 2 && function_exists('imagecreate') && function_exists('imagecolorset') && function_exists('imagecopyresized') &&
	...
	function_exists('imageline') && function_exists('imagecreatefromstring') && 
...
function seccode_background() {
	...
	$backgrounds = array();
	if($seccodedata['background'] && function_exists('imagecreatefromjpeg') && 
...
function seccode_ttffont() {
	...
	Common::debug($seccodettf , " : seccodettf");
	Common::debug($seccode, " : seccode");
	$seccodelength = 4;
